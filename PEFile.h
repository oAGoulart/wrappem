...
public:
  static const int SECTION_DESCRIPTOR_SIZE = 40;
...
public:
  PEFile(const char* filename)
  {
        ...
        (data_[0] == 0x5a && data_[1] == 0x4d))
    {
      ...
      if (data_[headerOffset_] == 0x50 && data_[headerOffset_ + 1] == 0x45)
      {
        ...
        uint16_t machine = *reinterpret_cast<uint16_t*>(&data_[headerOffset_ + MACHINE_OFFSET]);
        if (machine == 0x014c)
        {
          std::cout << "\t" << __c(42, "[+]") <<
        ...
        uint16_t numSections = *reinterpret_cast<uint16_t*>(&data_[headerOffset_ + SECTION_NUM_OFFSET]);
        std::cout << '\t' << __c(42, "[+]") <<
          "\tNumber of sections: " << numSections << std::endl;

        constexpr int OPT_HEADER_SIZE_OFFSET = SECTION_NUM_OFFSET + 14;
        uint16_t optHeaderSize = *reinterpret_cast<uint16_t*>(&data_[headerOffset_ + OPT_HEADER_SIZE_OFFSET]);
        std::cout << '\t' << __c(42, "[+]") <<
          "\tSize of optional header: " << optHeaderSize << std::endl;

        constexpr int OPT_HEADER_OFFSET = OPT_HEADER_SIZE_OFFSET + 4;
        uint16_t optHeaderMagic= *reinterpret_cast<uint16_t*>(&data_[headerOffset_ + OPT_HEADER_OFFSET]);
        bool isMagic64;
        ...
        if (is64_ != isMagic64)
          throw std::runtime_error("Optional header magic does not match machine");

        constexpr int DATADIR_COUNT = 16;
        constexpr int DATADIR_SIZE = 8;
        int dataDirsOffset = OPT_HEADER_OFFSET + optHeaderSize - DATADIR_COUNT * DATADIR_SIZE;
        importDataDirRVAOffset_ = dataDirsOffset + 8;
        uint32_t importDataDirRVA = *reinterpret_cast<uint32_t*>(&data_[headerOffset_ + importDataDirRVAOffset_]);
        importDataDirSizeOffset_ = importDataDirRVAOffset_ + 4;
        uint32_t importDataDirSize = *reinterpret_cast<uint32_t*>(&data_[headerOffset_ + importDataDirSizeOffset_]);
        ...
        for (uint16_t i = 0; i < numSections; ++i)
        {
          ...
          std::cout << '\t' << __c(42, "[+]") <<
            "\tFound section: " << section.name << std::endl;
        }
             ...
             section != sectionsParams_.cend(); ++section)
        {
          ...
          {
            uint32_t importDataDirOffset = importDataDirRVA - section->virtualAddress + section->rawDataAddress;
            importData_ = new char[importDataDirSize];
            importDataSize_ = importDataDirSize;
            if (memcpy_s(importData_, importDataDirSize, data_ + importDataDirOffset, importDataDirSize) != 0)
              throw std::runtime_error("Memory copying issue");

            std::cout << '\t' << __c(42, "[+]") <<
              "\tImport found at section: " << section->name << std::endl;
                  ...
                  std::string dummyFunc)
  {
    ...
    uint32_t v = static_cast<uint32_t>(baseAddress + importDataSize_ + 40 +
    ...
    memcpy_s(&buff[n], size - n, &v, 4);
    n += 4;
    v = static_cast<uint32_t>(baseAddress + importDataSize_ + 24 +
      payloadDll.length() + dummyFunc.length());
    memcpy_s(&buff[n], size - n, &v, 4);
    n += 4;
    memset(&buff[n], '\0', 20);
    ...
    v = static_cast<uint32_t>(baseAddress + importDataSize_ + 21 +
      payloadDll.length());
    memcpy_s(&buff[n], size - n, &v, 4);
    n += 4;
    memset(&buff[n], '\0', 12);
    ...
    uint32_t beginAlign = sectionsParams_.back().rawDataAddress + Align_(originalSize, 0x1000);
    ...
    uint32_t newImportSize = *reinterpret_cast<uint32_t*>(&data_[headerOffset_ + importDataDirSizeOffset_]);
    newImportSize += 0x14;
    memcpy_s(&data_[headerOffset_ + importDataDirSizeOffset_], dataSize_, &newImportSize, 4);
    FixSizeOfImage_();
  }
  ...
  void Save(char* filename)
  {
    ...
    if (!dirPath.empty()) MkDir(&dirPath.front());
...
private:
  ...
  {
    uint32_t size = sectionsParams_.back().virtualSize + sectionsParams_.back().virtualAddress;
    if (size % 0x1000 != 0)
  ...
  {
    return static_cast<uint32_t>(importDataSize_ + 56 +
      payloadDll.length() + dummyFunc.length());
  }
